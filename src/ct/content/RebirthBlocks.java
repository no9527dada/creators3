...
import CtCoreSystem.CoreSystem.type.MultiCrafter.CT3NoChooseMulti;
import CtCoreSystem.CoreSystem.type.MultiCrafter.CreatorsRecipe;
import CtCoreSystem.CoreSystem.type.Ovulam5480.CTMendProjector;
import arc.Core;
...
import static CtCoreSystem.CoreSystem.Evn2.*;
import static CtCoreSystem.content.CTFragShader.m;
...
public class RebirthBlocks {
    ...
    public static void load() {
        ...
        new Wall("移动平台") {
            ...
            {
                size = 4;
                requirements(Category.defense, with(物品, 1));
            }
        };
...
/*        破碎炉=new CT3NoChooseMulti("破碎炉",3){{
            size=1;
            addRecipe(
                    new CreatorsRecipe.InputContents(ItemStack.with(jin, 1), 2),
                    new CreatorsRecipe.OutputContents(ItemStack.with(jinfen, 2)), 22.0f
                    );
        }};*/
        破碎炉 = new GenericCrafter("破碎炉") {{
            size = 1;
            outputItem = new ItemStack(月银沙, 2);
            consumeItems(with(缕, 1
            ));
            consumePower(0.7f);
            requirements(Category.crafting, with(翠, 30, 灵玉, 10));
...
///电力
        ...
        修补器 = new CTMendProjector("修补器") {{
            ...
            consumePower(0.5f);
            health = 120;
            consumeItem(灵玉).boost();
            phaseRangeBoost = 24.0F;
            phaseBoost = 40.0F;
            requirements(Category.effect, with(灵玉, 20, 血晶, 30));
