package ct.content;

import arc.Core;
import arc.graphics.Color;
import CtCoreSystem.CoreSystem.type.CT3UnitType;
import mindustry.ai.types.BuilderAI;
import mindustry.content.UnitTypes;
import mindustry.type.UnitType;

import static mindustry.content.UnitTypes.*;

//Unit1.核心机0号.speed=3
//Unit1.核心机0号.killable = true
public class RebirthCoreUnit {
    public static UnitType 核心机0号, 核心机1号, 核心机2号, 核心机3号, 建造机;

    public static void load() {
        nova.playerControllable = false;
        核心机0号 = new CT3UnitType.CTR1UnitType("U0", "gamma") {{
            localizedName = Core.bundle.get("Unit.U0");
            description = Core.bundle.getOrNull("Unit.description.U0");
            aiController = BuilderAI::new;
            isEnemy = false;
            lowAltitude = true;
            flying = true;
            mineSpeed = 3f;
            mineTier = 1;
            buildSpeed = 0.5f;
            drag = 0.05f;
            speed = 0f;
            rotateSpeed = 15f;
            accel = 0.1f;
            itemCapacity = 30;
            health = 50f;
            engineOffset = 3f;
            hitSize = 8f;
            alwaysUnlocked = true;
            mineRange = 18 * 8f;//采矿范围
            hittable = false;//被子弹击中
            killable = false;//被杀死
            targetable = false;//被敌人瞄准
            physics = false;//单位碰撞
            lightColor = Color.valueOf("ffcd35");
            lightOpacity = 1;
        }};
        核心机1号 = new CT3UnitType.CTR1UnitType("U1", "elude") {{
            localizedName = Core.bundle.get("Unit.U1");
            description = Core.bundle.getOrNull("Unit.description.U1");
            health = 700;
            coreUnitDock = true;//true 解除附身后在其上方出现，而不是核心
            aiController = BuilderAI::new;
            flying = false;
            speed = 2.2f;
            mineSpeed = 3f;//采矿速度
            mineTier = 1;//采矿等级
            buildSpeed = 0.5f;//建造速度
            buildRange = 27.5f * 8F;//建造范围
            armor = 1f;
            hitSize = 11f;
            engineOffset = 7f;
            engineSize = 2f;
            hovering = true;//悬停
            itemCapacity = 200;
            useEngineElevation = false;
            mineWalls = false;//开采墙壁矿物
            mineFloor = true;//开采地表矿物
            lightRadius = 30 * 8;
            lightOpacity = 0.5f;

        }};
        核心机2号 = new CT3UnitType.CTR1UnitType("U2", "toxopid") {{
            localizedName = Core.bundle.get("Unit.U2");
            description = Core.bundle.getOrNull("Unit.description.U2");
            health = 1000;
            coreUnitDock = true;//true 解除附身后在其上方出现，而不是核心
            aiController = BuilderAI::new;

            flying = false;
            hovering = true;//悬停
            legCount = 0;//设定蜘蛛类，脚设定0个。实现可以踩方块不能过地图墙
            speed = 2.5f;
            mineSpeed = 4f;//采矿速度
            mineTier = 1;//采矿等级
            buildSpeed = 1f;//建造速度
            buildRange = 27.5f * 8F;//建造范围
            armor = 3f;
            hitSize = 8f;
            engineOffset = 7f;
            engineSize = 2f;
            itemCapacity = 240;
            useEngineElevation = false;
            mineWalls = false;//开采墙壁矿物
            mineFloor = true;//开采地表矿物
            lightRadius = 40 * 8;
            lightOpacity = 0.5f;
        }};

        核心机3号 = new UnitType("U3") {{
            constructor = flare.constructor;
            localizedName = Core.bundle.get("Unit.U3");
            description = Core.bundle.getOrNull("Unit.description.U3");
            health = 1500;
            coreUnitDock = true;//true 解除附身后在其上方出现，而不是核心
            aiController = BuilderAI::new;
            speed = 2.5f;
            boostMultiplier = 1.8f;//助推时速度倍率
            canBoost = true;//助推
            flying = false;
            // lowAltitude = true;
            // hovering = true;//悬停
            mineSpeed = 5f;//采矿速度
            mineTier = 2;//采矿等级
            buildSpeed = 1.5f;//建造速度
            buildRange = 30f * 8F;//建造范围
            armor = 3f;
            hitSize = 8f;
            engineOffset = 7f;
            engineSize = 2f;
            itemCapacity = 241;
            useEngineElevation = false;
            mineWalls = false;//开采墙壁矿物
            mineFloor = true;//开采地表矿物
            lightRadius = 40 * 8;
            lightOpacity = 0.5f;
            riseSpeed = 0.1F;//转身速度
            mechStepParticles = true;
            singleTarget = true;

/*
            mechFrontSway = 1f;
            mechStepParticles = true;
            stepShake = 0.15f;
            mechLandShake = 4f;
            singleTarget = true;

            targetable = true;
            physics = true;
            logicControllable = true;*/

        }};
    }
}
