...
//运输物流
        单位装载器1 = new UnitCargoLoader("单位装载器1") {{
            ...
            itemCapacity = 250;
            researchCost = with(冥石, 30);
        }};
        单位装载器2 = new UnitCargoLoader("单位装载器2") {{
            requirements(Category.distribution, with(冥石, 30));
            unitType = 运输2;
            size = 2;
            buildTime = 60f * 4f;
...
//
        ...
        反向溢流门4 = new OverflowGate("反向溢流门4") {{
            requirements(Category.distribution, with(Items.lead, 2, 冥石, 4));
            buildCostMultiplier = 3f;
            invert = true;
        }};
        //传送桥4
        传送桥4 = new ItemBridge("传送桥4") {{
            requirements(Category.distribution, with(冥石, 5, Items.silicon, 7, Items.lead, 10, Items.graphite, 10));
            range = 8;
            arrowPeriod = 0.9f;
            arrowTimeScl = 2.75f;
            hasPower = true;
        ...
        辉阳传送带 = new StackConveyor("辉阳传送带") {{
            requirements(Category.distribution, with(冥石, 1, Items.silicon, 1, Items.graphite, 1));
            health = 75;
            speed = 4f / 60f;
            itemCapacity = 10;
        }};
        ...
        辉阳路由器 = new StackRouter("辉阳路由器") {{
            requirements(Category.distribution, with(冥石, 5, Items.copper, 1));
            health = 130;
            speed = 6f;
            hasPower = true;
            consumesPower = true;
