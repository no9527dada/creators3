        ...
        Events.on(EventType.WorldLoadEvent.class, (ea) -> {
            if (Vars.state.isCampaign() && !enable) {
                ...
                Vars.ui.hudGroup.find("skip").clicked(() -> {
                    ...
                    float multiple = Float.parseFloat(Vars.state.rules.tags.get("增加敌对伤害生命倍数") == null ? "0.2" : Vars.state.rules.tags.get("增加敌对伤害生命倍数"));
                    if (multiple >= 0) {
                        Vars.state.rules.tags.put("增加敌对伤害生命倍数", String.valueOf(multiple + 0.2f));
                    }
